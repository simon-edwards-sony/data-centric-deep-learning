...
def get_ks_score(tr_probs, te_probs):
  ...
  statistic, p_value = ks_2samp(tr_probs, te_probs)
  score = p_value
  # ============================
  return score
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    tr_probs_cal = calibration_model.fit_transform(tr_probs, tr_labels)

    te_probs_cal = calibration_model.transform(te_probs)

    tr_probs_cal = torch.tensor(tr_probs_cal)
    te_probs_cal = torch.tensor(te_probs_cal)
    # ============================
